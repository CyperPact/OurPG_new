            ...
            if ($this->form_validation->run() == TRUE):
               $data_array =  array('username'=>$this->input->post('pg_contact_email'));  
                if($this->login->select_info('user_info',$data_array)):
                    echo 'Username is not available <i class="fa fa-times" aria-hidden="true"></i>';
                else :
                    echo "<label>Username</label><br/>".$this->input->post('pg_contact_email') ." <i class='icon-check-sign'></i>";
                endif;
...
$from = 'admin@cyberpact.in';
$to   = 'pvningalkar@gmail.com';
...
$config['smtp_host'] = EMAIL_SMTP_HOST;
$config['smtp_user'] = "admin@cyberpact.in";//"admin@cyberpact.in";
